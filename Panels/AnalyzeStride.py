...
class AnalyzeStridePanel(wx.Panel):
    def __init__(self, parent):
        ...
        self.right_to_left = bool(configs["right_to_left"])
        ...
        self.header = wx.StaticText(self, -1, "Kinematic Analysis", size=(500, 100))
        font = wx.Font(20, wx.MODERN, wx.NORMAL, wx.NORMAL)
        self.header.SetFont(font)
        self.sizer.Add(
            self.header, pos=(0, 0), span=(2, 4), flag=wx.LEFT | wx.TOP, border=25
        )

        self.instructions = wx.StaticText(
            self,
            -1,
            "Load the csv file of bodypart coordinates and extract kinematic parameters.",
            size=(self.window_width, 50),
        )
        font = wx.Font(15, wx.MODERN, wx.NORMAL, wx.NORMAL)
        self.instructions.SetFont(font)
        self.sizer.Add(
            self.instructions, pos=(2, 0), span=(1, 4), flag=wx.LEFT, border=25
        )
    ...
    def Stride_UI(self):
        ...
        self.analysis_type_rbox = wx.RadioBox(
            self,
            id=wx.ID_ANY,
            label="Kinematic experimental set-up",
        ...
        self.sizer.Add(
            self.import_folder_button, pos=(7, 0), flag=wx.LEFT | wx.TOP, border=25
        )
        self.stride_widgets.append(self.import_folder_button)

        self.import_folder_text = wx.StaticText(
            self,
            label="Import all csv files from a folder for stride extraction and calculate kinematics parameters. "
        ...
        self.sizer.Add(
            self.import_folder_text, pos=(7, 1), flag=wx.LEFT | wx.TOP, border=25
        )
    ...
    def onAnalysisTypeRadioBox(self, e):
        ...
        if self.analysis_type == "Treadmill":
            ...
            self.sizer.Add(
                self.method_note_text, pos=(9, 1), flag=wx.LEFT | wx.TOP, border=25
            )
            ...
            if self.pixels_per_cm == "":
                wx.MessageBox(
                    'For spontaneous walking analysis, please supply a value for "pixels_per_cm" in the config.yaml file!',
                    "Error",
    ...
    def ImportKinematicsCSV(self, e):
        ...
        if import_dialog.ShowModal() == wx.ID_OK:
            ...
            self.df, self.bodyparts = KinematicsFunctions.fix_column_names(self.df)
        ...
        try:
            if self.df is not None:
                ...
                self.onAnalysisTypeRadioBox(self)
                ...
                if self.pixels_per_cm != "":
                    ...
                    wx.MessageBox(
                        "For spontaneous walking, please be sure to supply a pxl-cm conversion ratio in the config file!",
                        "Note",
                        wx.OK | wx.ICON_INFORMATION,
                    )
        ...
        if self.analysis_type == "Treadmill":
            ...
            if self.method_selection == "Semi-automated":
                ...
                try:
                    ...
                    self.cm_speed_input_button = wx.Button(
                        self, id=wx.ID_ANY, label="Update speed (cm/s)"
                    )
                    self.cm_speed_input_button.Bind(wx.EVT_BUTTON, self.UpdateSpeed)
                    self.sizer.Add(
                        self.cm_speed_input_button,
                        pos=(9, 3),
                        flag=wx.LEFT | wx.TOP,
                        border=25,
                    )
                ...
                try:
                    ...
                    self.px_cm_ratio_input = wx.TextCtrl(
                        self, value=str(self.px_to_cm_speed_ratio)
                    )
                    self.sizer.Add(
                        self.px_cm_ratio_input,
                        pos=(9, 2),
                        flag=wx.LEFT | wx.TOP,
                        border=25,
                    )
                    ...
                    self.px_cm_ratio_input_button = wx.Button(
                        self, id=wx.ID_ANY, label="Update pixel-to-cm speed ratio"
                    )
                    self.px_cm_ratio_input_button.Bind(wx.EVT_BUTTON, self.UpdateRatio)
                    self.sizer.Add(
                        self.px_cm_ratio_input_button,
                        pos=(9, 3),
                        flag=wx.LEFT | wx.TOP,
                        border=25,
                    )
            ...
            try:
                ...
                self.px_cm_length_ratio_input = wx.TextCtrl(
                    self, value=str(self.pixels_per_cm)
                )
                self.sizer.Add(
                    self.px_cm_length_ratio_input,
                    pos=(9, 2),
                    flag=wx.LEFT | wx.TOP,
                    border=25,
                )
                ...
                self.px_cm_length_ratio_input_button = wx.Button(
                    self, id=wx.ID_ANY, label="Update px/cm length ratio"
                )
                self.px_cm_length_ratio_input_button.Bind(
                    wx.EVT_BUTTON, self.UpdateLengthRatio
                )
                self.sizer.Add(
                    self.px_cm_length_ratio_input_button,
                    pos=(9, 3),
                    flag=wx.LEFT | wx.TOP,
                    border=25,
                )
    ...
    def EstimateParams(self, e):
        if self.has_imported_file:
            ...
            if self.analysis_type == "Treadmill":
                if self.method_selection == "Semi-automated":
                    (
                        self.est_cm_speed,
                        self.est_px_speed,
                        self.est_pixels_per_cm,
                        self.est_px_to_cm_speed_ratio,
                    ) = KinematicsFunctions.estimate_speed(
                        self.df,
                        "toe",
                        self.cm_speed,
                        None,
            ...
            try:
                ...
                self.sizer.Add(
                    self.select_output_path_button,
                    pos=(11, 1),
                    flag=wx.TOP | wx.LEFT | wx.BOTTOM,
                    border=25,
                )
                self.stride_widgets.append(self.select_output_path_button)
                self.select_output_path_button.Bind(
                    wx.EVT_BUTTON, self.SelectOutputPath
                )
                self.select_output_path_button.Show()
            try:
    ...
    def SelectOutputFolder(self, e):
        export_dialog = wx.DirDialog(
            self,
            "Choose a folder to save parameter extraction results",
            self.dirname,
            style=wx.DD_DEFAULT_STYLE,
        )
        if export_dialog.ShowModal() == wx.ID_OK:
            self.output_path = export_dialog.GetPath()
            self.extract_parameters_text.SetLabel(
                f"Saving to {self.output_path}. You can now start parameter extraction. \n\n"
        ...
        try:
            ...
            self.sizer.Add(
                self.bulk_extract_parameters_button,
                pos=(11, 2),
                flag=wx.TOP | wx.BOTTOM,
                border=25,
            )
            self.stride_widgets.append(self.bulk_extract_parameters_button)
            self.bulk_extract_parameters_button.Bind(
                wx.EVT_BUTTON, self.BulkExtractParameters
            )
    ...
    def BulkExtractParameters(self, e):
        for i, file in enumerate(self.files):
            self.GetParent().SetStatusText(
                f"Extracting parameters for file {i+1} out of {len(self.files)}..."
            )
            self.GetParent().Layout()
            ...
            plot_path = os.path.join(
                self.output_path, f'truncated_stickplot_{file.replace(".csv", ".svg")}'
            )
            if self.analysis_type == "Treadmill":
                if self.method_selection == "Semi-automated":
                    (
                ...
                parameters, pd_dataframe_coords, is_stance, bodyparts = (
                    KinematicsFunctions.extract_spontaneous_parameters(
                        self.frame_rate,
                        self.df,
                        self.cutoff_f,
                        self.pixels_per_cm,
                        self.no_outlier_filter,
                        self.dragging_filter,
                    )
                )
                parameters_truncated = KinematicsFunctions.return_continuous(
                    parameters,
            ...
            KinematicsFunctions.make_parameters_output(
                os.path.join(self.output_path, f"parameters_{file}"), parameters
            )
            KinematicsFunctions.make_parameters_output(
                os.path.join(self.output_path, f"continuous_strides_parameters_{file}"),
                parameters_truncated,
            )

        KinematicsFunctions.make_averaged_output(
            self.output_path
        )  # average of all steps per animal
        KinematicsFunctions.make_averaged_output(
            self.output_path, truncated=True
        )  # average of continuous steps per animal
    ...
    def SelectOutputPath(self, e):
        export_dialog = wx.FileDialog(
            self,
            "Save parameter extraction results to... ",
            self.dirname,
            "",
            "CSV files (*.csv)|*.csv|All files(*.*)|*.*",
        ...
        if export_dialog.ShowModal() == wx.ID_OK:
            ...
            try:
                ...
                self.sizer.Add(
                    self.extract_parameters_button,
                    pos=(11, 2),
                    flag=wx.TOP | wx.BOTTOM,
                    border=25,
                )
                self.stride_widgets.append(self.extract_parameters_button)
                self.extract_parameters_button.Bind(
                    wx.EVT_BUTTON, self.ExtractParameters
                )
            self.extract_parameters_button.Show()
            self.GetParent().Layout()
        else:
    ...
    def ExtractParameters(self, e):
        ...
        if self.analysis_type == "Treadmill":
            ...
            KinematicsFunctions.make_parameters_output(
                self.output_path, parameters_truncated
            )
