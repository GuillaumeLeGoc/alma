...
def filter_predictions(
    ...
    """
    result = np.where(
        pd_dataframe.iloc[list(t_peaks)][f"{bodypart} likelihood"]
    ...
    for i in range(1, len(result)):
        ...
        max_between_slip = min(
            pd_dataframe.iloc[prev_mid:curr_mid][
                pd_dataframe.iloc[prev_mid:curr_mid][f"{bodypart} likelihood"]
                >= likelihood_threshold
            ][f"{bodypart} y"]
        )
        max_between = np.where(
            pd_dataframe.iloc[prev_mid:curr_mid][f"{bodypart} y"] == max_between_slip
        )[0][0]
        max_between = np.array(
            pd_dataframe.iloc[prev_mid:curr_mid]["bodyparts coords"]
        )[max_between]
        prev_depth = (
            pd_dataframe.iloc[t_peaks[result[i - 1]]][f"{bodypart} y"]
            - pd_dataframe.iloc[properties["left_bases"][result[i - 1]]][
                f"{bodypart} y"
            ]
        )
        ...
        if curr_start > prev_end:
            ...
            if curr_mid_depth - max_between_slip >= depth_threshold * prev_depth:
                ...
                if curr_mid_depth > prev_mid_depth:
                    ...
                    ind_valid_peaks.pop()
                    ind_valid_peaks.append(result[i])
                    properties["left_bases"][result[i]] = properties["left_bases"][
                        result[i - 1]
                    ]
                else:
                    # prev prediction is deeper; adjust end of slip timing
                    properties["right_bases"][result[i - 1]] = properties[
                        "right_bases"
                    ][result[i]]
...
def find_footfalls(
    ...
    **kwargs,
    ...
    if method == "Deviation":
        ...
        """
        t_peaks, properties = find_peaks(
            pd_dataframe[f"{bodypart} {axis}"], height=-10000, prominence=(45, 100000)
        )
        ...
        index = np.array(
            pd_dataframe[
                pd_dataframe[f"{bodypart} likelihood"] >= likelihood_threshold
            ].index
        )
        ...
        """
        if threshold == "":
            threshold = np.mean(pd_dataframe[f"{bodypart} {axis}"]) + np.std(
                pd_dataframe[f"{bodypart} {axis}"]
            )
...
def calculate_depths(pd_dataframe, bodypart, starts, ends, t_peaks):
    if ends == []:
        depth = np.array(pd_dataframe[f"{bodypart} y"][t_peaks]) - np.array(
            pd_dataframe[f"{bodypart} y"][starts]
        )
    else:
        depth = (
            (
            ...
            + (
                np.array(pd_dataframe[f"{bodypart} y"][t_peaks])
                - np.array(pd_dataframe[f"{bodypart} y"][ends])
            )
        ) / 2
...
def plot_frame(
    video_file, n_frame, width, height, frame_rate, pd_dataframe, bodypart, zoom
):
    try:
...
def plot_labels(
    ...
    axes.scatter(
        pd_dataframe["bodyparts coords"], pd_dataframe[f"{bodypart} {axis}"], s=1
    )
    axes.scatter(
    ...
    axes.scatter(
        pd_dataframe[pd_dataframe[f"{bodypart} likelihood"] < likelihood_threshold][
            "bodyparts coords"
        ],
        pd_dataframe[pd_dataframe[f"{bodypart} likelihood"] < likelihood_threshold][
            f"{bodypart} {axis}"
    ...
    for i, t in enumerate(t_pred):
        ...
        if confirmed[i]:
            ...
            c = "g"
            axes.annotate(
                str(count_footfalls), (t, pd_dataframe[f"{bp} {axis}"][t]), color=c
            )
        else:
            pass
...
def find_confirmed_neighbors(
    n_current_frame, t_val, confirmed=None, start=None, end=None
):
...
def DisplayPlots(panel, set_bodypart=True):
    ...
    try:
        ...
        graph = plot_labels(
            ...
            panel.bodypart,
            panel.bodypart_list_val,
            panel.selected_bodyparts,
            "y",
            panel.likelihood_threshold,
            panel.confirmed,
            panel.zoom,
        )
