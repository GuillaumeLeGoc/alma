...
def treadmill_correction(pd_dataframe, bodyparts, px_speed=8.09):
    ...
    if type(bodyparts) is list and len(bodyparts) > 1:
        for bodypart in bodyparts:
            pd_dataframe[f"{bodypart} x"] = -(
                pd_dataframe[f"{bodypart} x"] - correction
            )

    elif type(bodyparts) is list and len(bodyparts) == 1:
        pd_dataframe[f"{bodyparts[0]} x"] = -(
            pd_dataframe[f"{bodyparts[0]} x"] - correction
        )
...
def estimate_speed(
    ...
    bodypart,
    cm_speed,
    px_to_cm_speed_ratio,
    frame_rate,
    right_to_left=True,
):
    x_change = np.diff(
        pd_dataframe[f"{bodypart} x"][pd_dataframe[f"{bodypart} likelihood"] > 0.5]
    )
    x_change_filt = x_change[
...
def angle(vector1, vector2):
    ...
    return math.acos(
        (vector1[0] * vector2[0] + vector1[1] * vector2[1]) / (length1 * length2)
    )
...
def find_swing_stance(elevation_angle_change, start, end):
    ...
    try:
        ...
        stance_dur = np.where(np.diff(np.sign(elevation_angle_change[start:end])) > 0)[
            0
        ][0]
    except IndexError:
    ...
    return (
        stance_dur,
        (end - start - stance_dur) / (end - start),
        stance_dur / (end - start),
    )
...
def find_drag(toe_y, stance_dur, threshold, start, end):
    ...
    return sum(np.where(is_drag, [1], [0])), sum(np.where(is_drag, [1], [0])) / (
        end - stance_dur
    )
...
def find_euclidean_speed(x, y, frame_rate):
    ...
    for i in range(1, len(x)):
        loc_changes.append(
            np.sqrt((x[i] - x[i - 1]) ** 2 + (y[i] - y[i - 1]) ** 2) * frame_rate
        )
    return loc_changes
...
def parse_stride_lists(x, starts, ends, first_stride, n, adjust=False):
    ...
    """
    results = [
        x[starts[i] : ends[i] + 1] for i in range(first_stride, first_stride + n)
    ]
    if adjust:
...
def make_averaged_output(pathname, truncated=False):
    ...
    for file in os.listdir(pathname):
        if truncated:
            if file.endswith(".csv") and file.startswith(
                "continuous_strides_parameters_"
            ):
                files.append(file)
        else:
    ...
    res = res.drop(
        ["Unnamed: 0", "stride_start (frame)", "stride_end (frame)"],
        axis=1,
        errors="ignore",
    )
    if truncated:
...
def return_continuous(
    pd_dataframe_parameters,
    n_continuous=10,
    plot=False,
    pd_dataframe_coords=None,
    bodyparts=None,
    is_stance=[],
    filename="",
):
    print(pd_dataframe_parameters["cycle duration (s)"])
    valid_list = convert_to_binary(
        np.array(pd_dataframe_parameters["cycle duration (s)"], dtype="float32")
    )
    ...
    if end_stride == start_stride:
        ...
        try:
            if plot:
                x_coords, y_coords = collect_filtered_coords(
                    pd_dataframe_coords, bodyparts
                )
                start_frame = int(
                    pd_dataframe_parameters.iloc[start_stride]["stride_start (frame)"]
                )
                end_frame = int(
                    pd_dataframe_parameters.iloc[end_stride]["stride_end (frame)"]
                )
                continuous_stickplot(
                    pd_dataframe_coords,
                    bodyparts,
                    is_stance,
                    x_coords,
...
def extract_parameters(
    ...
    smooth_toe_x = butterworth_filter(
        pd_dataframe_corrected["toe x"], frame_rate, cutoff_f
    )
    smooth_mtp_x = butterworth_filter(
        pd_dataframe_corrected["mtp x"], frame_rate, cutoff_f
    )
    smooth_ankle_x = butterworth_filter(
        pd_dataframe_corrected["ankle x"], frame_rate, cutoff_f
    )
    smooth_knee_x = butterworth_filter(
        pd_dataframe_corrected["knee x"], frame_rate, cutoff_f
    )
    smooth_hip_x = butterworth_filter(
        pd_dataframe_corrected["hip x"], frame_rate, cutoff_f
    )
    smooth_crest_x = butterworth_filter(
        pd_dataframe_corrected["iliac_crest x"], frame_rate, cutoff_f
    )
    ...
    smooth_crest_y = butterworth_filter(
        pd_dataframe["iliac_crest y"], frame_rate, cutoff_f
    )
    ...
    if (
        np.abs(stat.median(x_change)) < 0.5
        and len(np.where(np.abs(x_change) < 2)[0]) / len(x_change) > 0.68
    ):
        ...
        print("Too much dragging? Please check raw data.")
        print(
            f"Median of selected bodypart x coordinate change per frame: {stat.median(x_change)}"
        )
        print("Calculating a subset of parameters independent of step cycles...")
        ...
        is_stance = False  # we'll use this as to flag failure to extract stride
        ...
        treadmill_y = float(
            stance_threshold + np.sqrt(gm.covariances_[ind_stance_gaussian].item())
        )
        ...
        for i in range(len(starts)):
            ...
            cycle_v = (
                stride_len / cycle_dur_frame * frame_rate
            )  # (px / frames) * (frames/s) = px/s

            stance_dur_frame, swing_perc, stance_perc = find_swing_stance(
                elevation_angle_change, starts[i], ends[i]
            )
            ...
            if swing_dur_sec and stance_dur_frame > 0:
                ...
                step_height = (
                    -(
                        min(smooth_toe_y[starts[i] + stance_dur_frame : ends[i]])
                        - max(smooth_toe_y[starts[i] : starts[i] + stance_dur_frame])
                    )
                    / pixels_per_cm
                )
            ...
            if (
                ...
                limb_len_means.append(
                    np.mean(limb_lens[starts[i] : ends[i]]) / pixels_per_cm
                )
                limb_len_maxs.append(limb_len_max)
                limb_len_mins.append(
                    min(limb_lens[starts[i] : ends[i]]) / pixels_per_cm
                )
                limb_len_sds.append(
                    np.std(limb_lens[starts[i] : ends[i]]) / pixels_per_cm
                )
                ...
                if stance_dur_frame == cycle_dur_frame:
                    ...
                    max_v_during_swings.append(
                        max(velocities[starts[i] : ends[i]]) / pixels_per_cm
                    )
                ...
                ankle_joint_sds.append(
                    np.std(angles_mtp_ankle_knee[starts[i] : ends[i]])
                )
                knee_joint_sds.append(
                    np.std(angles_ankle_knee_hip[starts[i] : ends[i]])
                )
                ...
                drag, drag_percent = find_drag(
                    smooth_toe_y, stance_dur_frame, treadmill_y, starts[i], ends[i]
                )
        ...
        for i in range(len(starts)):
            if starts[i] not in starts_included:
                ...
                included_index = int(
                    np.array(np.where(starts[i] == starts_included)[0]).item()
                )

                if included_index <= len(starts_included) - 5:
                    toe_xs = parse_stride_lists(
                        smooth_toe_x,
                        starts_included,
                        ends_included,
                        included_index,
                    ...
                    toe_xys = [
                        np.append(toe_xs[j], toe_ys[j]).reshape(2, -1).T
                        for j in range(5)
                    ]
        ...
        for bodypart in bodyparts:
            for coord in ["x", "y"]:
                pd_dataframe[f"{bodypart} {coord}"] = butterworth_filter(
                    pd_dataframe[f"{bodypart} {coord}"], frame_rate, cutoff_f
                )
...
def extract_spontaneous_parameters(
    frame_rate,
    pd_dataframe,
    cutoff_f,
    ...
    bodyparts = [
        ...
        "hip",
        "iliac_crest",
    ]  # should be modified to detect names / change this manually on error!
    starts_all = []
    ends_all = []
    limbs = []
    ...
    for direction in ["L", "R", ""]:
        if f"toe{direction} x" in pd_dataframe.columns:
            x_change = np.diff(
                pd_dataframe[f"toe{direction} x"][
                    pd_dataframe[f"toe{direction} likelihood"] > 0.5
                ]
            )
            # print('found', direction)
            smooth_toe_x = butterworth_filter(
                pd_dataframe[f"toe{direction} x"], frame_rate, cutoff_f
            )
            smooth_mtp_x = butterworth_filter(
                pd_dataframe[f"mtp{direction} x"], frame_rate, cutoff_f
            )
            smooth_ankle_x = butterworth_filter(
                pd_dataframe[f"ankle{direction} x"], frame_rate, cutoff_f
            )
            smooth_knee_x = butterworth_filter(
                pd_dataframe[f"knee{direction} x"], frame_rate, cutoff_f
            )
            smooth_hip_x = butterworth_filter(
                pd_dataframe[f"hip{direction} x"], frame_rate, cutoff_f
            )
            smooth_crest_x = butterworth_filter(
                pd_dataframe[f"iliac_crest{direction} x"], frame_rate, cutoff_f
            )

            smooth_toe_y = butterworth_filter(
                pd_dataframe[f"toe{direction} y"], frame_rate, cutoff_f
            )
            smooth_mtp_y = butterworth_filter(
                pd_dataframe[f"mtp{direction} y"], frame_rate, cutoff_f
            )
            smooth_ankle_y = butterworth_filter(
                pd_dataframe[f"ankle{direction} y"], frame_rate, cutoff_f
            )
            smooth_knee_y = butterworth_filter(
                pd_dataframe[f"knee{direction} y"], frame_rate, cutoff_f
            )
            smooth_hip_y = butterworth_filter(
                pd_dataframe[f"hip{direction} y"], frame_rate, cutoff_f
            )
            smooth_crest_y = butterworth_filter(
                pd_dataframe[f"iliac_crest{direction} y"], frame_rate, cutoff_f
            )
            ...
            limb_lens = find_limb_length(
                smooth_toe_x, smooth_toe_y, smooth_hip_x, smooth_hip_y
            )
            velocities = find_euclidean_speed(smooth_toe_x, smooth_toe_y, frame_rate)
            ...
            if (
                dragging_filter
                and len(np.where(np.abs(elevation_angle_change) < 1)[0])
                / len(elevation_angle_change)
                > 0.68
            ):
                ...
                for i in range(1, len(is_stance)):
                    if is_stance[i] != is_stance[i - 1]:
                        if (
                            is_stance[i]
                            and pd_dataframe[f"toe{direction} likelihood"][i] > 0.6
                        ):
                            if len(starts) == 0:
                                starts.append(i)
                            elif (
                                abs(
                                    pd_dataframe[f"toe{direction} x"][starts[-1]]
                                    - pd_dataframe[f"toe{direction} x"][i]
                                )
                                > 20
                            ):
                                starts.append(i)
                ...
                y = pd_dataframe[f"toe{direction} y"][
                    pd_dataframe[f"toe{direction} likelihood"] > 0.5
                ]
                y_filt = y[
                    (y < np.mean(y) + 1 * np.std(y)) & (y > np.mean(y) - 1 * np.std(y))
                ]
                _, b = np.histogram(y_filt, bins=100, density=True)
                # fit 2 Gaussians to the pdf of toe y coord
                gm = GaussianMixture(n_components=2, random_state=0).fit(
                ...
                treadmill_y = float(
                    stance_threshold
                    + np.sqrt(gm.covariances_[np.where(gm.means_ == stance_threshold)])
                )
                ...
                for i in range(len(starts)):
                    ...
                    stride_len = abs(
                        find_stride_len(smooth_toe_x, starts[i], ends[i])
                    )  # in pixel
                    stride_len_cm = stride_len / pixels_per_cm
                    limb_len_max = max(limb_lens[starts[i] : ends[i]]) / pixels_per_cm
                    ...
                    cycle_dur_frame = ends[i] - starts[i]
                    cycle_v = (
                        stride_len_cm / cycle_dur_frame * frame_rate
                    )  # (px / frames) * (frames/s) = px/s
                    ...
                    if swing_dur_sec and stance_dur_frame > 0:
                        ...
                        step_height = (
                            -(
                                min(smooth_toe_y[starts[i] : ends[i]])
                                - max(smooth_toe_y[starts[i] : ends[i]])
                            )
                            / pixels_per_cm
                        )
                    elif swing_dur_sec and stance_dur_frame == 0:
                        step_height = (
                            -(
                                min(
                                    smooth_toe_y[starts[i] + stance_dur_frame : ends[i]]
                                )
                                - smooth_toe_y[starts[i]]
                            )
                            / pixels_per_cm
                    ...
                    if no_outlier_filter or (
                        ...
                        mtp_joint_extension = max(
                            angles_toe_mtp_ankle[starts[i] : ends[i]]
                        )
                        ankle_joint_extension = max(
                            angles_mtp_ankle_knee[starts[i] : ends[i]]
                        )
                        knee_joint_extension = max(
                            angles_ankle_knee_hip[starts[i] : ends[i]]
                        )
                        hip_joint_extension = max(
                            angles_knee_hip_crest[starts[i] : ends[i]]
                        )

                        mtp_joint_flexion = min(
                            angles_toe_mtp_ankle[starts[i] : ends[i]]
                        )
                        ankle_joint_flexion = min(
                            angles_mtp_ankle_knee[starts[i] : ends[i]]
                        )
                        knee_joint_flexion = min(
                            angles_ankle_knee_hip[starts[i] : ends[i]]
                        )
                        hip_joint_flexion = min(
                            angles_knee_hip_crest[starts[i] : ends[i]]
                        )

                        mtp_joint_amplitude = mtp_joint_extension - mtp_joint_flexion
                        ankle_joint_amplitude = (
                            ankle_joint_extension - ankle_joint_flexion
                        )
                        knee_joint_amplitude = knee_joint_extension - knee_joint_flexion
                        hip_joint_amplitude = hip_joint_extension - hip_joint_flexion

                        mtp_joint_sds.append(
                            np.std(angles_toe_mtp_ankle[starts[i] : ends[i]])
                        )
                        ankle_joint_sds.append(
                            np.std(angles_mtp_ankle_knee[starts[i] : ends[i]])
                        )
                        ...
                        hip_joint_sds.append(
                            np.std(angles_knee_hip_crest[starts[i] : ends[i]])
                        )
    ...
    return (
        ...
        pd_dataframe,
        np.array(is_stances).reshape(-1),
        bodyparts,
    )


def collect_filtered_coords(filt_corrected_df, bodyparts):
    x_coords = filt_corrected_df[
        [
            f"{bodyparts[0]} x",
            f"{bodyparts[1]} x",
            f"{bodyparts[2]} x",
...
def continuous_stickplot(
    ...
    for t in filt_corrected_df.index[start:end]:
        if t % 2 == 0:
            ...
            toe_mtp = pd.DataFrame(
                pd.concat([x_coords[t].iloc[0:2], y_coords[t].iloc[0:2]], axis=1)
            )
            mtp_ankle = pd.DataFrame(
                pd.concat([x_coords[t].iloc[1:3], y_coords[t].iloc[1:3]], axis=1)
            )
            ankle_knee = pd.DataFrame(
                pd.concat([x_coords[t].iloc[2:4], y_coords[t].iloc[2:4]], axis=1)
            )
            knee_hip = pd.DataFrame(
                pd.concat([x_coords[t].iloc[3:5], y_coords[t].iloc[3:5]], axis=1)
            )
            hip_crest = pd.DataFrame(
                pd.concat([x_coords[t].iloc[4:6], y_coords[t].iloc[4:6]], axis=1)
            )
