...
class HomeFrame(wx.Frame):
    ...
    def makeMenuBar(self):
        ...
        analyze_footfall_item = limb_motion_menu.Append(
            -1, "&Footfall detection\tCtrl-L", "Detect footfalls and validate results"
        )
        analyze_stride_item = limb_motion_menu.Append(
            -1,
            "&Gait kinematic data extraction\tCtrl-K",
            "Extract gait kinematic parameters with bodypart coordinate data",
        )
        #################################
        analysis_menu = wx.Menu()
        random_forest_item = analysis_menu.Append(
            -1,
            "&Random forest classification\tCtrl-R",
            "Random forest classification using extracted gait kinematic parameters",
        )
        principal_component_analysis_item = analysis_menu.Append(
            -1, "&PCA\tCtrl-P", "PCA using extracted gait kinematic parameters"
        )
        #################################
        help_menu = wx.Menu()
    ...
    def on_random_forest(self, e):
        ...
        self.SetStatusText(
            "Run random forest classification on extracted gait kinematic parameters"
        )
        self.Layout()
        self.Refresh()  # refresh to show slider in right proportion
